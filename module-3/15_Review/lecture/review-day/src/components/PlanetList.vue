<template>
  <div>
      <ol>
          <li v-for="(planet, index) in $store.state.planets" :key="index" v-on:click="placeOnTopByIdx(index)">
              {{ planet }}
          </li>
      </ol>
      <input type="text" ref="planetInput" v-model="newPlanet" v-on:keyup.enter="addPlanet('input')"/>
      <button type="button" @click.prevent="addPlanet('button')">Add Planet</button>
  </div>
</template>
